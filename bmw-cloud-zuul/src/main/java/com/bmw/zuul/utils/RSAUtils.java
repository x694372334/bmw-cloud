...
public class RSAUtils {  
    ...
    */
    ...
    public static RSAPublicKey getPublicKey(String allName){
    	...
    	try {
    		//本地环境用这种
   		 	fin = new FileInputStream((RSAUtils.class.getResource("/rsa/") + allName).replaceFirst("file:", ""));
   		 
   		 
   		 	//打jar包方式用这种
//   		 fin = new FileInputStream("/data/rsa/"+ allName);
...
//   		 fin = new FileInputStream("/data/rsa/"+ allName);
    ...
    public static void convertRSAToPEM(boolean type,String orinPath){
    	if(  orinPath.isEmpty() || orinPath == null){
    		log.info("非法参数！"); 
    		return;
    	}
    	FileInputStream fin = null;
