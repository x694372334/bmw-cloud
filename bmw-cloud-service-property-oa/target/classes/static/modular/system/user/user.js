...
MgrUser.initColumn = function () {
    var columns = [
        ...
        {title: '是否注册环信', field: 'huanxinFlag', align: 'center', valign: 'middle', sortable: true},
...
//注册环信
MgrUser.registeredHuanxin = function () {
	if (this.check()) {
		if("是"==this.seItem.huanxinFlag){
			 Feng.info("已注册!");
			return;
		}
	 parent.layer.confirm('确认注册环信？', {
         btn: ['确定', '取消'],
         shade: false //不显示遮罩
     }, function () {
    	 var userId = MgrUser.seItem.id;
         var ajax = new $ax(Feng.ctxPath + "/mgr/registeredHuanxin", function (data) {
             Feng.success("注册成功!");
...
//注销环信
MgrUser.dropHuanxin = function () {
	if (this.check()) {
		if("否"==this.seItem.huanxinFlag){
			 Feng.info("未注册!");
			return;
		}
	 parent.layer.confirm('确认注销环信？', {
         btn: ['确定', '取消'],
         shade: false //不显示遮罩
     }, function () {
    	 var userId = MgrUser.seItem.id;
         var ajax = new $ax(Feng.ctxPath + "/mgr/dropHuanxin", function (data) {
             Feng.success("注销成功!");
             MgrUser.table.refresh();
         }, function (data) {
             Feng.error("注销失败!");
...
MgrUser.search = function () {
    ...
    queryData['parenteid'] = MgrUser.parentEId;
...
MgrUser.showEnterpriseSelectTree = function () {
	Feng.showInputTree("eId", "enterTreeDiv", 15, 34);
};

MgrUser.onClickEnter = function (e, treeId, treeNode) {
	MgrUser.parentEId = treeNode.id;
    MgrUser.search();
};
...
$(function () {
    ...
    var ztree = new $ZTree("deptTree", "/propertyDepartment/selectdepartmentTreeList");
    ztree.bindOnClick(MgrUser.onClickDept);
    ztree.init();
    
    //初始化企业树
    var enterztree = new $ZTree("enterpriseTree", "/enterpriseInfo/selectEnterpriseParentTreeList");
    enterztree.bindOnClick(MgrUser.onClickEnter);
    enterztree.init();
    MgrUser.enterztreeInstance = enterztree;
