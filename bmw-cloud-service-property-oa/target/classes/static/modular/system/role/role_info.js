...
RolInfoDlg.onClickDept = function (e, treeId, treeNode) {
  ...
  //查询所属物业
    var ajax = new $ax(Feng.ctxPath + "/propertyDepartment/detail/"+treeNode.id, function (data) {
    	$("#ename").val(data.eName);
    }, function (data) {
        Feng.error("查询部门失败!" + data.responseJSON.message + "!");
    });
    ajax.start();
...
RolInfoDlg.onClickPName = function (e, treeId, treeNode) {
    ...
    $("#pid").val(treeNode.id);
  //查询父级所属部门
    var ajax = new $ax(Feng.ctxPath + "/role/findDept/"+$("#pid").val(), function (data) {
    	var obj = JSON.parse(data);
        $("#deptName").attr("value", obj.name);
        $("#deptid").attr("value", obj.id);
    }, function (data) {
        Feng.error("查询部门失败!" + data.responseJSON.message + "!");
    });
    ajax.start();
...
RolInfoDlg.collectData = function () {
    this.set('id').set('name').set('pid').set('deptid').set('tips').set('num').set('version').set('parentEId').set('type');
};
...
RolInfoDlg.addSubmit = function () {
    ...
    console.log(this.roleInfoData.pid);
    console.log(this.roleInfoData.parentEId);
 ...
 * 点击父级编号input框时
 */
RolInfoDlg.onClickEnter = function (e, treeId, treeNode) {
	$("#parentEName").attr("value", RolInfoDlg.enterztreeInstance.getSelectedVal());
    $("#parentEId").attr("value",treeNode.pcode);
};

RolInfoDlg.showEnterpriseSelectTree = function () {
    Feng.showInputTree("parentEId", "enterTreeDiv", 15, 34);
};

$(function () {
    Feng.initValidator("roleInfoForm", RolInfoDlg.validateFields);

    var deptTree = new $ZTree("deptTree", "/propertyDepartment/selectdepartmentTreeList");
   ...
   // $("#deptName").attr("placeholder","请先选择上级");
    ...
    if ($("#parentEName").length > 0) {
		// 初始化企业树
		var enterztree = new $ZTree("enterpriseTree","/enterpriseInfo/selectEnterpriseParentTreeList");
		enterztree.bindOnClick(RolInfoDlg.onClickEnter);
		enterztree.init();
		RolInfoDlg.enterztreeInstance = enterztree;
	}
    
    
    if($("#deptid").val()!=null&&""!=$("#deptid").val()!=""){
    	//查询所属物业
        var ajax = new $ax(Feng.ctxPath + "/propertyDepartment/detail/"+$("#deptid").val(), function (data) {
        	$("#ename").val(data.eName);
        }, function (data) {
            Feng.error("查询部门失败!" + data.responseJSON.message + "!");
        });
